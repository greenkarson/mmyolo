...
class PPYOLOEModel(torch.nn.Module):
...
def parse_args():
    parser = argparse.ArgumentParser(
        description='convert ppyoloe model to ONNX.')
    parser.add_argument(
        '--config', type=str, 
        default='configs/ppyoloe/ppyoloe_plus_s_fast_8xb8-80e_coco.py',
        help='ppyoloe config file path from mmdetection.')
    parser.add_argument(
        '--checkpoint',
        type=str,
        default='ppyoloe_plus_s_fast_8xb8-80e_coco_20230101_154052-9fee7619.pth.1',
        help='ppyoloe checkpoint path from mmdetection.')
    parser.add_argument('--output', type=str, default='ppyoloe_plus_s_fast.onnx', help='output filename.')
    parser.add_argument(
        '--device',
        type=str,
        default='cpu',
...
if __name__ == '__main__':
    ...
    rtmdet = PPYOLOEModel(model, args.device)
    rtmdet.eval()
